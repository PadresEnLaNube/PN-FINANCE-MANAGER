(function($) {
    ...
    class PN_FINANCE_MANAGER_Selector {
        ...
        createStructure() {
            const wrapper = $('<div class="pn-finance-manager-selector"></div>');
            const control = $('<div class="pn-finance-manager-selector__control"></div>');
            const valueContainer = $('<div class="pn-finance-manager-selector__value-container"></div>');
            const input = $('<input type="text" class="pn-finance-manager-selector__input" />');
            const indicator = $('<span class="pn-finance-manager-selector__indicator"><i class="material-icons-outlined pn-finance-manager-selector__indicator-icon">keyboard_arrow_down</i></span>');
            
            valueContainer.append(input);
            control.append(valueContainer, indicator);
            wrapper.append(control);
            
            this.menu = $('<div class="pn-finance-manager-selector__menu" style="display: none;"></div>');
            
            if (this.options.searchable) {
                const searchContainer = $('<div class="pn-finance-manager-selector__search"></div>');
            ...
            this.optionsContainer = $('<div class="pn-finance-manager-selector__options"></div>');
        ...
        bindEvents() {
            ...
            this.menu.on('click', '.pn-finance-manager-selector__option', (e) => {
        ...
        openMenu() {
            ...
            this.wrapper.find('.pn-finance-manager-selector__indicator-icon').text('keyboard_arrow_up');
        }
        ...
        closeMenu() {
            ...
            this.wrapper.find('.pn-finance-manager-selector__indicator-icon').text('keyboard_arrow_down');
        }
        ...
        updateOptions() {
            ...
            options.each((_, option) => {
                ...
                const optionElement = $('<div class="pn-finance-manager-selector__option"></div>')
                    .text(label)
                    .data('value', value);
        ...
        filterOptions(searchTerm) {
            const options = this.optionsContainer.find('.pn-finance-manager-selector__option');
        ...
        addSelectedValue(value, label) {
            ...
            this.valueContainer.find('.pn-finance-manager-selector__placeholder').remove();
            if (this.options.multiple) {
                const valueElement = $('<div class="pn-finance-manager-selector__multi-value"></div>');
                const labelElement = $('<span class="pn-finance-manager-selector__multi-value__label"></span>').text(label);
                const removeButton = $('<span class="pn-finance-manager-selector__multi-value__remove"><i class="material-icons-outlined pn-finance-manager-icon-close">close</i></span>');
                ...
                removeButton.on('click', (e) => {
                    ...
                    this.optionsContainer.find('.pn-finance-manager-selector__option').each(function() {
                ...
                const valueElement = $('<span class="pn-finance-manager-selector__single-value"></span>');
                const labelElement = $('<span class="pn-finance-manager-selector__single-value__label"></span>').text(label);
                const removeButton = $('<span class="pn-finance-manager-selector__single-value__remove"><i class="material-icons-outlined pn-finance-manager-icon-close">close</i></span>');
                removeButton.on('click', (e) => {
                    e.stopPropagation();
                    // Clear selection
                    this.selectedValues = [];
                    ...
                    if (this.placeholder) {
                        this.input.hide();
                        this.valueContainer.append('<span class="pn-finance-manager-selector__placeholder">' + this.placeholder + '</span>');
                    }
                });
                valueElement.append(labelElement, removeButton);
                this.valueContainer.append(valueElement);
    ...
    $.fn.PN_FINANCE_MANAGER_Selector = function(options) {
        return this.each(function() {
            if (!$(this).data('-selector')) {
                $(this).data('-selector', new PN_FINANCE_MANAGER_Selector(this, options));
            }
        });
    };
