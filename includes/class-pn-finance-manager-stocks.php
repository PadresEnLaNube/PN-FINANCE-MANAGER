...
class PN_FINANCE_MANAGER_Stocks {
	 ...
	 * @since    1.0.5
	 */
	public function pn_finance_manager_update_stock_symbols_from_api_cron() {
		$api_enabled = get_option('pn_finance_manager_stocks_api_enabled', '');
		if ($api_enabled !== 'on') {
			error_log('PnFinanceManager: Stocks API not enabled. Aborting symbol update.');
			return new WP_Error('api_config_error', 'Stocks API not enabled.');
		}

		// Use the cache update method
		$result = $this->pn_finance_manager_update_stock_symbols_cache();
		
		if ($result && is_array($result)) {
			error_log('PnFinanceManager: Stock symbols updated via cron successfully. Total symbols: ' . count($result));
			return true;
		} else {
			error_log('PnFinanceManager: Failed to update stock symbols via cron.');
	 ...
	 */
	public function pn_finance_manager_get_stock_symbols_for_form() {
		error_log('PnFinanceManager Debug: pn_finance_manager_get_stock_symbols_for_form() called');
		...
		error_log('PnFinanceManager Debug: Cache check - cached_symbols count: ' . count($cached_symbols));
		error_log('PnFinanceManager Debug: Cache check - last_update: ' . $last_update);
		error_log('PnFinanceManager Debug: Cache check - cache_expiry: ' . $cache_expiry);
		error_log('PnFinanceManager Debug: Cache check - current_time: ' . current_time('timestamp'));
		
		// Check if cache is valid
		if (!empty($cached_symbols) && is_array($cached_symbols) && 
			($last_update + $cache_expiry) > current_time('timestamp')) {
			error_log('PnFinanceManager Debug: Returning cached symbols: ' . count($cached_symbols));
			return $cached_symbols;
		}
		
		error_log('PnFinanceManager Debug: Cache invalid or expired, trying to update');
		
		// If cache is empty or expired, try to update it
		$update_result = $this->pn_finance_manager_update_stock_symbols_cache();
		
		if ($update_result && !empty($update_result)) {
			error_log('PnFinanceManager Debug: Cache update successful, returning: ' . count($update_result));
			return $update_result;
		}
		
		error_log('PnFinanceManager Debug: Cache update failed, checking for old cache');
		
		// If update failed, return cached data if available, otherwise fallback
		if (!empty($cached_symbols) && is_array($cached_symbols)) {
			error_log('PnFinanceManager Debug: Returning old cached symbols: ' . count($cached_symbols));
			return $cached_symbols;
		}
		
		error_log('PnFinanceManager Debug: No cache available, returning popular symbols');
		
		// Final fallback to popular symbols
		return $this->pn_finance_manager_get_popular_symbols();
	}
	 ...
	 */
	public function pn_finance_manager_update_stock_symbols_cache() {
		...
		$symbols = $this->pn_finance_manager_get_twelvedata_symbols();
		
		if (!empty($symbols) && is_array($symbols)) {
			// Store in cache
			update_option('pn_finance_manager_stock_symbols_cache', $symbols, false);
			...
			error_log('PnFinanceManager: Stock symbols cache updated successfully. Total symbols: ' . count($symbols));
	 ...
	 */
	private function pn_finance_manager_get_twelvedata_symbols() {
		$url = 'https://api.twelvedata.com/stocks?country=United%20States&type=Common%20Stock';

		$response = wp_remote_get($url, [
			'timeout' => 120,
			'user-agent' => 'PnFinanceManager/1.1'
		]);

		if (is_wp_error($response)) {
			error_log('PnFinanceManager: Twelve Data stocks API error: ' . $response->get_error_message());
			return $this->pn_finance_manager_get_popular_symbols();
		}
		...
		if (empty($data['data']) || !is_array($data['data'])) {
			error_log('PnFinanceManager: Twelve Data stocks API returned empty or invalid data.');
			return $this->pn_finance_manager_get_popular_symbols();
		}
	 ...
	 */
	private function pn_finance_manager_get_popular_symbols() {
		return [
			'AAPL' => 'Apple Inc.',
	 ...
	 */
	public function pn_finance_manager_get_stock_data($symbol) {
		// Check if stocks API is enabled
		$api_enabled = get_option('pn_finance_manager_stocks_api_enabled', '');
		if ($api_enabled !== 'on') {
			return false;
		...
		$cache_duration = get_option('pn_finance_manager_stocks_cache_duration', 3600);
		...
		$cache_key = 'pn_finance_manager_stock_data_twelvedata_' . $symbol;
		...
		$data = $this->pn_finance_manager_get_twelvedata_quote($api_key, $symbol);
	 ...
	 * @return   array|false           Stock data or false on failure.
	 */
	private function pn_finance_manager_get_twelvedata_quote($api_key, $symbol) {
		$url = "https://api.twelvedata.com/quote?symbol=" . urlencode($symbol) . "&apikey=" . urlencode($api_key);

		$exchange_map = get_option('pn_finance_manager_stock_exchange_map', []);
		...
		if (is_wp_error($response)) {
			error_log('PnFinanceManager: Twelve Data quote error: ' . $response->get_error_message());
			return false;
		}
		...
		if (empty($data) || isset($data['code']) || empty($data['close'])) {
			...
			error_log('PnFinanceManager: Twelve Data quote API error: ' . $error_msg);
			return false;
		}
	 ...
	 */
	public function pn_finance_manager_display_stock($symbol) {
		// Placeholder for stock display functionality
		echo '<div class="pn-finance-manager-stock-display">Stock: ' . esc_html($symbol) . '</div>';
	}
	 ...
	 */
	public function pn_finance_manager_get_stock_price($symbol) {
		$data = $this->pn_finance_manager_get_stock_data($symbol);
	 ...
	 */
	public function pn_finance_manager_get_stock_chart($symbol, $period = '1m') {
	 ...
	 */
	public function pn_finance_manager_get_historical_stock_data($symbol, $days = 30, $user_id = null) {
		if ($user_id === null) {
		...
		$existing_data = $this->pn_finance_manager_get_stock_price_history($symbol, $days, $user_id);
		$last_update = get_option('pn_finance_manager_historical_data_last_update_' . $symbol, '');
		...
		$historical_data = $this->pn_finance_manager_get_twelvedata_historical_data($api_key, $symbol, $days);

		if ($historical_data) {
			$this->pn_finance_manager_store_historical_data($symbol, $historical_data, $user_id);
			update_option('pn_finance_manager_historical_data_last_update_' . $symbol, current_time('mysql'), false);
	 ...
	 * @return   array|false           Historical data or false on failure.
	 */
	private function pn_finance_manager_get_twelvedata_historical_data($api_key, $symbol, $days) {
		...
		if (is_wp_error($response)) {
			error_log('PnFinanceManager: Twelve Data time_series error: ' . $response->get_error_message());
			return false;
		}
		...
		if (empty($data['values']) || !is_array($data['values']) || isset($data['code'])) {
			$error_msg = isset($data['message']) ? $data['message'] : 'Unknown error';
			error_log('PnFinanceManager: Twelve Data time_series API error: ' . $error_msg);
			return false;
		}
	 ...
	 */
	private function pn_finance_manager_store_historical_data($symbol, $historical_data, $user_id = null) {
		if (empty($historical_data)) {
			return false;
		}
		
		$option_key = 'pn_finance_manager_stock_price_history_' . strtoupper($symbol);
	 ...
	 * @return   bool                 Success status.
	 */
	public function pn_finance_manager_record_stock_price($symbol, $data, $user_id = null) {
		$option_key = 'pn_finance_manager_stock_price_history_' . strtoupper($symbol);
	 ...
	 */
	public function pn_finance_manager_get_stock_price_history($symbol, $days = 30, $user_id = null) {
		$option_key = 'pn_finance_manager_stock_price_history_' . strtoupper($symbol);
		$price_history = get_option($option_key, []);
	 ...
	 */
	public function pn_finance_manager_get_stock_price_for_date($symbol, $date, $user_id = null) {
		$option_key = 'pn_finance_manager_stock_price_history_' . strtoupper($symbol);
		$price_history = get_option($option_key, []);
	 ...
	 * @since    1.0.5
	 */
	public function pn_finance_manager_daily_stock_price_recording() {
		...
		$stock_assets = get_posts([
			...
			'meta_query' => [
				...
				[
					'relation' => 'OR',
					[
						'key' => 'pn_finance_manager_asset_sold',
						'compare' => 'NOT EXISTS'
					],
					[
						'key' => 'pn_finance_manager_asset_sold',
		...
		foreach ($stock_assets as $asset) {
			$symbol = get_post_meta($asset->ID, 'pn_finance_manager_stock_symbol', true);
		...
		$watchlist = new PN_FINANCE_MANAGER_Watchlist();
		$all_users = get_users(['fields' => 'ID']);
		foreach ($all_users as $uid) {
			$wl_items = $watchlist->pn_finance_manager_watchlist_get_user_items($uid);
		...
		foreach ($user_symbols as $user_id => $symbols) {
			foreach ($symbols as $symbol) {
				...
				$stock_data = $this->pn_finance_manager_get_stock_data($symbol);

				if ($stock_data) {
					$result = $this->pn_finance_manager_record_stock_price($symbol, $stock_data, $user_id);
					if ($result) {
						$recorded_count++;
					} else {
						$error_count++;
	 ...
	 */
	public function pn_finance_manager_get_user_assets($user_id) {
		...
		$user_assets = get_posts([
			...
			'meta_query' => [
				'relation' => 'OR',
				[
					'key' => 'pn_finance_manager_owners',
					'value' => $user_id,
					'compare' => 'LIKE'
				],
				[
					'key' => 'pn_finance_manager_owners_checkbox',
					'value' => 'on',
					'compare' => '!='
				],
				[
		...
		foreach ($unique_assets as $asset) {
			$asset_type = get_post_meta($asset->ID, 'pn_finance_manager_asset_type', true);
			
			if (empty($asset_type) || !isset($organized_assets[$asset_type])) {
				$asset_type = 'other';
			}
			
			$is_sold = get_post_meta($asset->ID, 'pn_finance_manager_asset_sold', true) === 'on';
			$sold_price = floatval(get_post_meta($asset->ID, 'pn_finance_manager_asset_sold_price', true));
			$sold_date = get_post_meta($asset->ID, 'pn_finance_manager_asset_sold_date', true);

			$asset_data = [
				'id' => $asset->ID,
				'title' => $asset->post_title,
				'description' => $asset->post_content,
				'purchase_date' => get_post_meta($asset->ID, 'pn_finance_manager_asset_date', true),
				'purchase_time' => get_post_meta($asset->ID, 'pn_finance_manager_asset_time', true),
				'current_value' => 0,
				'purchase_value' => 0,
				'profit_loss' => 0,
			...
			if ($asset_type === 'stocks') {
				...
				if (!empty($symbol)) {
					...
					$asset_data['sector'] = get_post_meta($asset->ID, 'pn_finance_manager_stock_sector', true);
					$asset_data['country'] = get_post_meta($asset->ID, 'pn_finance_manager_stock_country', true);
					...
					if ($is_sold && $sold_price > 0) {
						...
						if ($stock_data) {
							...
							$asset_data['price_history'] = $this->pn_finance_manager_get_stock_price_history($symbol, $history_days, $user_id);
						}
					}
					$total_amount = get_post_meta($asset->ID, 'pn_finance_manager_stock_total_amount', true);
					$purchase_price = get_post_meta($asset->ID, 'pn_finance_manager_stock_purchase_price', true);
					$purchase_date = get_post_meta($asset->ID, 'pn_finance_manager_asset_date', true);

					if (empty($purchase_price) || floatval($purchase_price) == 0) {
						if ($purchase_date) {
							$purchase_price_data = $this->pn_finance_manager_get_stock_price_for_date($symbol, $purchase_date, $user_id);
							if ($purchase_price_data && isset($purchase_price_data['price'])) {
								$purchase_price = floatval($purchase_price_data['price']);
								update_post_meta($asset->ID, 'pn_finance_manager_stock_purchase_price', $purchase_price);
							} else {
			...
			elseif ($asset_type === 'real_estate') {
				...
				$re_currency = get_option('pn_finance_manager_currency', 'eur');
				$re_rate = PN_FINANCE_MANAGER_Data::pn_finance_manager_get_usd_exchange_rate($re_currency);
			...
			elseif ($asset_type === 'cryptocurrencies') {
				$coin_id = get_post_meta($asset->ID, 'pn_finance_manager_crypto_symbol', true);
				if (!empty($coin_id)) {
					...
					if ($is_sold && $sold_price > 0) {
						...
						$crypto_data = $this->pn_finance_manager_get_crypto_data($coin_id);
						if ($crypto_data) {
						...
						$asset_data['price_history'] = $this->pn_finance_manager_get_crypto_price_history($coin_id, $history_days);
					}

					$amount = get_post_meta($asset->ID, 'pn_finance_manager_crypto_amount', true);
					$purchase_price = get_post_meta($asset->ID, 'pn_finance_manager_crypto_purchase_price', true);
					$purchase_date = get_post_meta($asset->ID, 'pn_finance_manager_asset_date', true);

					// Try to resolve purchase price from history if == 0
					if (empty($purchase_price) || floatval($purchase_price) == 0) {
						if ($purchase_date) {
							$purchase_price_data = $this->pn_finance_manager_get_crypto_price_for_date($coin_id, $purchase_date);
							if ($purchase_price_data && isset($purchase_price_data['price'])) {
								$purchase_price = floatval($purchase_price_data['price']);
								update_post_meta($asset->ID, 'pn_finance_manager_crypto_purchase_price', $purchase_price);
							} else {
								if (!empty($asset_data['price_history'])) {
									$first_record = reset($asset_data['price_history']);
									if (isset($first_record['price'])) {
	 ...
	 */
	public function pn_finance_manager_get_user_liabilities($user_id) {
		...
		$balance_fields = [
			'mortgage'      => 'pn_finance_manager_mortgage_remaining_balance',
			'car_loan'      => 'pn_finance_manager_car_loan_remaining_balance',
			'student_loan'  => 'pn_finance_manager_student_loan_remaining_balance',
			'credit_card'   => 'pn_finance_manager_credit_card_balance',
			'personal_loan' => 'pn_finance_manager_personal_loan_remaining_balance',
			'medical_debt'  => 'pn_finance_manager_medical_debt_remaining_balance',
			'business_loan' => 'pn_finance_manager_business_loan_remaining_balance',
			'tax_debt'      => 'pn_finance_manager_tax_debt_remaining_balance',
			'other'         => 'pn_finance_manager_other_liability_amount',
		];
		...
		foreach ($user_liabilities as $liability) {
			$liability_type = get_post_meta($liability->ID, 'pn_finance_manager_liability_type', true);
			...
			$date = get_post_meta($liability->ID, 'pn_finance_manager_liability_date', true);
	 ...
	 */
	private function pn_finance_manager_compute_period_change($asset, $period) {
	 ...
	 */
	public function pn_finance_manager_display_user_assets($user_id) {
		$user_assets = $this->pn_finance_manager_get_user_assets($user_id);
		$user_liabilities = $this->pn_finance_manager_get_user_liabilities($user_id);

		if (empty($user_assets) && empty($user_liabilities)) {
			return '<div class="pn-finance-manager-user-assets-empty">' .
				   '<p>' . __('No assets found.', 'pn-finance-manager') . '</p>' .
				   '</div>';
		}

		ob_start();
		$currency = get_option('pn_finance_manager_currency', 'eur');
		$comparison_period = get_user_meta($user_id, 'pn_finance_manager_comparison_period', true);
		if (empty($comparison_period)) {
			$comparison_period = 'daily';
		}
		$usd_to_currency = PN_FINANCE_MANAGER_Data::pn_finance_manager_get_usd_exchange_rate($currency);
		?>
		<div class="pn-finance-manager-user-assets-portfolio">
			<div class="pn-finance-manager-portfolio-summary pn-finance-manager-mb-50">
				<h2 class="pn-finance-manager-section-toggle collapsed"><?php esc_html_e('Financial Overview', 'pn-finance-manager'); ?> <i class="material-icons-outlined pn-finance-manager-section-toggle-icon">expand_less</i></h2>
				<div class="pn-finance-manager-section-body">
				<?php
				...
				$net_worth = $total_portfolio_value - $total_liabilities_value;

				$converted_portfolio_value = PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($total_portfolio_value, $currency);
				$converted_liabilities_value = PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($total_liabilities_value, $currency);
				$converted_net_worth = PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($net_worth, $currency);
				$converted_portfolio_pnl = PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($total_profit_loss, $currency);
				$converted_global_invested = PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($global_total_invested, $currency);
				$formatted_portfolio_value = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency($converted_portfolio_value, $currency);
				$formatted_liabilities_value = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency($converted_liabilities_value, $currency);
				$formatted_net_worth = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency($converted_net_worth, $currency);
				$formatted_portfolio_pnl = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency($converted_portfolio_pnl, $currency);
				$formatted_global_invested = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency($converted_global_invested, $currency);
				?>
				<div class="pn-finance-manager-performance-metrics pn-finance-manager-global-metrics">
					<div class="pn-finance-manager-metric-row">
						<div class="pn-finance-manager-metric-item">
							...
							<span class="pn-finance-manager-metric-value positive">
								<?php echo esc_html($formatted_portfolio_value['full']); ?>
							</span>
						</div>
						<div class="pn-finance-manager-metric-item">
							<span class="pn-finance-manager-metric-label"><?php esc_html_e('Total Invested', 'pn-finance-manager'); ?></span>
							<span class="pn-finance-manager-metric-value">
					...
					<div class="pn-finance-manager-metric-row">
						<div class="pn-finance-manager-metric-item">
							...
							<span class="pn-finance-manager-metric-value negative">
								<?php echo esc_html($formatted_liabilities_value['full']); ?>
							</span>
						</div>
						<div class="pn-finance-manager-metric-item">
							<span class="pn-finance-manager-metric-label"><?php esc_html_e('Net Worth', 'pn-finance-manager'); ?></span>
							<span class="pn-finance-manager-metric-value <?php echo esc_attr($net_worth >= 0 ? 'positive' : 'negative'); ?>">
					...
					<div class="pn-finance-manager-metric-row">
						<div class="pn-finance-manager-metric-item">
							<span class="pn-finance-manager-metric-label"><?php esc_html_e('Profit/Loss', 'pn-finance-manager'); ?></span>
							<span class="pn-finance-manager-metric-value <?php echo esc_attr($total_profit_loss >= 0 ? 'positive' : 'negative'); ?>">
						...
						<div class="pn-finance-manager-metric-item">
							...
							?>
							<span class="pn-finance-manager-metric-label"><?php esc_html_e('Total Return', 'pn-finance-manager'); ?></span>
							<span class="pn-finance-manager-metric-value <?php echo esc_attr($global_return_pct >= 0 ? 'positive' : 'negative'); ?>">
				...
				$dist_values = [];
				$dist_colors = [];
				$dist_grand_total = $total_portfolio_value + $total_liabilities_value;
				...
				foreach ($user_assets as $dist_key => $dist_data) {
					if ($dist_data['total_value'] > 0) {
						...
						$dist_values[] = round(PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($dist_data['total_value'], $currency), 2);
				...
				foreach ($user_liabilities as $dist_key => $dist_data) {
					if ($dist_data['total_value'] > 0) {
						$dist_labels[] = $dist_data['label'];
						$dist_values[] = round(-1 * PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($dist_data['total_value'], $currency), 2);
						$dist_colors[] = $liability_colors_list[$lc_idx % count($liability_colors_list)];
						$lc_idx++;
					}
				}
				// Net Worth total bar
				$dist_labels[] = __('Net Worth', 'pn-finance-manager');
				$dist_values[] = round(PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($net_worth, $currency), 2);
				$dist_colors[] = $net_worth >= 0 ? '#007bff' : '#dc3545';
				if (!empty($dist_values)):
				$dist_currency_symbol = PN_FINANCE_MANAGER_Data::pn_finance_manager_get_currency_symbol($currency);
				$dist_grand_converted = PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($dist_grand_total, $currency);
				?>
				<div class="pn-finance-manager-distribution-chart" style="margin-top:20px;">
					<h4><?php esc_html_e('Asset & Liability Distribution', 'pn-finance-manager'); ?></h4>
					<div class="pn-finance-manager-chart-container" style="height:350px;box-sizing:border-box;">
						<canvas id="pn-finance-manager-global-distribution-chart"></canvas>
					</div>
				</div>
				<script>
				jQuery(document).ready(function($) {
					function drawGlobalDistributionChart() {
						...
						var ctx = document.getElementById('pn-finance-manager-global-distribution-chart').getContext('2d');
						var labels = <?php echo json_encode($dist_labels); ?>;
						var dataValues = <?php echo json_encode($dist_values); ?>;
						var bgColors = <?php echo json_encode($dist_colors); ?>;
						var grandTotal = <?php echo json_encode($dist_grand_converted); ?>;
						var symbol = '<?php echo esc_js($dist_currency_symbol); ?>';
						new Chart(ctx, {
							type: 'bar',
							data: {
								labels: labels,
			...
			if ($has_stocks || $has_crypto) {
				...
				$converted_invested = PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($total_invested, $currency);
				$converted_current = PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($current_value, $currency);
				$converted_profit = PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($profit_loss, $currency);
				...
				$formatted_invested = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency($converted_invested, $currency);
				$formatted_current = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency($converted_current, $currency);
				$formatted_profit = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency($converted_profit, $currency);

				// --- Breakdown doughnut charts (inline row) ---
				$breakdown_symbol = PN_FINANCE_MANAGER_Data::pn_finance_manager_get_currency_symbol($currency);
				...
				if (isset($user_assets['stocks'])) {
					foreach ($user_assets['stocks']['assets'] as $s_asset) {
						...
						if ($s_val > 0) {
							$s_converted = round(PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($s_val, $currency), 2);
				...
				if (isset($user_assets['cryptocurrencies']) && !empty($user_assets['cryptocurrencies']['assets'])) {
					foreach ($user_assets['cryptocurrencies']['assets'] as $c_asset) {
						...
						if ($c_val > 0) {
							...
							$crypto_chart_values[] = round(PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($c_val, $currency), 2);
						}
					}
				}
				...
				if (isset($user_assets['real_estate']) && !empty($user_assets['real_estate']['assets'])) {
					foreach ($user_assets['real_estate']['assets'] as $re_asset) {
						...
						if ($re_val > 0) {
							...
							$re_chart_values[] = round(PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($re_val, $currency), 2);
						}
					}
				}
				...
				if ($has_stocks) {
					...
					if (!empty($stocks_perf_history)) {
						$conv_sp_inv = PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($stocks_perf_invested, $currency);
						$conv_sp_cur = PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($stocks_perf_current, $currency);
						$fmt_sp_inv = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency($conv_sp_inv, $currency);
						$fmt_sp_cur = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency($conv_sp_cur, $currency);
						$stocks_perf_chart_data = [];
						foreach ($stocks_perf_history as $date => $value) {
							$stocks_perf_chart_data[] = [
								'recorded_date' => $date,
								'value' => PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($value, $currency)
							];
						}
					}
				}
				...
				if ($has_crypto) {
					...
					if (!empty($crypto_perf_history)) {
						$conv_cp_inv = PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($crypto_perf_invested, $currency);
						$conv_cp_cur = PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($crypto_perf_current, $currency);
						$fmt_cp_inv = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency($conv_cp_inv, $currency);
						$fmt_cp_cur = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency($conv_cp_cur, $currency);
			...
			?>
				</div><!-- .pn-finance-manager-section-body -->
			</div><!-- .pn-finance-manager-portfolio-summary -->

			<div class="pn-finance-manager-section-header">
				<h2 class="pn-finance-manager-section-toggle collapsed"><?php esc_html_e('Assets', 'pn-finance-manager'); ?> <i class="material-icons-outlined pn-finance-manager-section-toggle-icon">expand_less</i></h2>
				...
				<a href="#" class="pn-finance-manager-popup-open-ajax pn-finance-manager-text-decoration-none" data-pn-finance-manager-popup-id="pn-finance-manager-popup-pn_finance_manager_asset-add" data-pn-finance-manager-ajax-type="pn_finance_manager_asset_new">
					<i class="material-icons-outlined pn-finance-manager-cursor-pointer pn-finance-manager-vertical-align-middle pn-finance-manager-tooltip pn-finance-manager-color-main-0" title="<?php esc_attr_e('Add new Asset', 'pn-finance-manager'); ?>">add_circle</i>
				</a>
				<?php endif; ?>
			</div>
			<div class="pn-finance-manager-section-body">
			...
			<div class="pn-finance-manager-performance-metrics pn-finance-manager-portfolio-metrics">
				<div class="pn-finance-manager-metric-row">
					<div class="pn-finance-manager-metric-item">
						<span class="pn-finance-manager-metric-label"><?php esc_html_e('Total Invested', 'pn-finance-manager'); ?></span>
						<span class="pn-finance-manager-metric-value portfolio-total-invested">
							<?php echo esc_html($formatted_invested['full']); ?>
						</span>
					</div>
					<div class="pn-finance-manager-metric-item">
						<span class="pn-finance-manager-metric-label"><?php esc_html_e('Current Value', 'pn-finance-manager'); ?></span>
						<span class="pn-finance-manager-metric-value portfolio-current-value">
				...
				<div class="pn-finance-manager-metric-row">
					<div class="pn-finance-manager-metric-item">
						<span class="pn-finance-manager-metric-label"><?php esc_html_e('Profit/Loss', 'pn-finance-manager'); ?></span>
						<span class="pn-finance-manager-metric-value portfolio-profit-loss <?php echo esc_attr($profit_loss >= 0 ? 'positive' : 'negative'); ?>">
							<?php echo esc_html(($profit_loss >= 0 ? '+' : '-') . $formatted_profit['full']); ?>
						</span>
					</div>
					<div class="pn-finance-manager-metric-item">
						<span class="pn-finance-manager-metric-label"><?php esc_html_e('Total Return', 'pn-finance-manager'); ?></span>
						<span class="pn-finance-manager-metric-value portfolio-total-return <?php echo esc_attr($total_return >= 0 ? 'positive' : 'negative'); ?>">
			...
			<?php foreach ($user_assets as $type_key => $type_data): ?>
				<div class="pn-finance-manager-asset-category">
					<h3 class="pn-finance-manager-section-toggle collapsed"><?php echo esc_html($type_data['label']); ?>
						<span class="pn-finance-manager-asset-count">(<?php echo intval($type_data['count']); ?>)</span>
						<i class="material-icons-outlined pn-finance-manager-section-toggle-icon">expand_less</i>
					</h3>
					<div class="pn-finance-manager-section-body">
					<div class="pn-finance-manager-sort-bar">
						<select class="pn-finance-manager-sort-select">
							<option value="name"><?php esc_html_e('Name', 'pn-finance-manager'); ?></option>
							<option value="value"><?php esc_html_e('Value', 'pn-finance-manager'); ?></option>
							<?php if ($type_key === 'stocks' || $type_key === 'cryptocurrencies' || $type_key === 'real_estate'): ?>
							<option value="pl"><?php esc_html_e('Profit/Loss', 'pn-finance-manager'); ?></option>
							<?php endif; ?>
							<option value="date"><?php esc_html_e('Purchase Date', 'pn-finance-manager'); ?></option>
						</select>
						<button type="button" class="pn-finance-manager-sort-dir-btn" data-dir="asc" title="<?php esc_attr_e('Sort ascending', 'pn-finance-manager'); ?>">
							<i class="material-icons-outlined">arrow_upward</i>
						</button>
					</div>

					<?php if ($type_key === 'stocks' || $type_key === 'cryptocurrencies'): ?>
						<div class="pn-finance-manager-stocks-grid">
							<?php foreach ($type_data['assets'] as $asset): ?>
								<div class="pn-finance-manager-stock-card<?php echo esc_attr(!empty($asset['is_sold']) ? ' pn-finance-manager-stock-card-sold' : ''); ?> pn-finance-manager-asset" data-pn_finance_manager_asset-id="<?php echo esc_attr($asset['id']); ?>" data-sort-name="<?php echo esc_attr($asset['title']); ?>" data-sort-value="<?php echo esc_attr($asset['current_total_value']); ?>" data-sort-pl="<?php echo esc_attr($asset['profit_loss']); ?>" data-sort-date="<?php echo esc_attr($asset['purchase_date']); ?>" data-sort-sold="<?php echo esc_attr(!empty($asset['is_sold']) ? '1' : '0'); ?>">
									<?php
									$asset_pct = ($type_data['total_value'] > 0 && isset($asset['current_total_value']))
										...
										: 0;
									?>
									<div class="pn-finance-manager-stock-header pn-finance-manager-stock-toggle">
										<h4>
											<?php echo esc_html($asset['title']); ?>
											<?php if ($asset_pct > 0): ?>
												<span class="pn-finance-manager-asset-pct"><?php echo esc_html(number_format($asset_pct, 1)); ?>%</span>
											<?php endif; ?>
											<?php if (!empty($asset['symbol_name'])): ?>
												<span class="pn-finance-manager-stock-symbol-name"><?php echo esc_html($asset['symbol_name']); ?></span>
											<?php endif; ?>
										</h4>
										<div>
											<span class="pn-finance-manager-stock-symbol"><?php echo esc_html(strtoupper($asset['symbol'])); ?></span>
											<i class="material-icons-outlined pn-finance-manager-stock-toggle-icon">expand_more</i>
										</div>
									</div>

									<?php if ($asset['stock_data']): ?>
										<div class="pn-finance-manager-stock-card-summary pn-finance-manager-mb-10">
											<div class="pn-finance-manager-stock-card-summary-value">
												<?php
												$converted = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency(PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($asset['current_total_value'], $currency), $currency);
												echo esc_html($converted['full']);
												?>
											</div>
											<?php if (!empty($asset['is_sold'])): ?>
												<div class="pn-finance-manager-stock-change pn-finance-manager-sold-badge">
													<?php esc_html_e('Sold', 'pn-finance-manager'); ?>
												</div>
											<?php else: ?>
												<?php
												$period_change = $this->pn_finance_manager_compute_period_change($asset, $comparison_period);
												$badge_total_change = $period_change['change'] * floatval($asset['shares']);
												$badge_converted = PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($badge_total_change, $currency);
												$badge_formatted = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency(abs($badge_converted), $currency);
												?>
												<div class="pn-finance-manager-stock-change <?php echo esc_attr($badge_converted >= 0 ? 'positive' : 'negative'); ?>">
													<?php echo $badge_converted >= 0 ? '+' : '-'; ?><?php echo esc_html($badge_formatted['full']); ?>
													(<?php echo esc_html($period_change['change_percent']); ?>)
													<span class="pn-finance-manager-period-label"><?php echo esc_html($period_change['label']); ?></span>
												</div>
											<?php endif; ?>
										</div>
									<?php endif; ?>

									<div class="pn-finance-manager-stock-body" style="display:none;">
										<div class="pn-finance-manager-stock-body-actions">
											<a href="#" class="pn-finance-manager-popup-open-ajax pn-finance-manager-text-decoration-none pn-finance-manager-edit-asset-link" data-pn-finance-manager-popup-id="pn-finance-manager-popup-pn_finance_manager_asset-edit" data-pn-finance-manager-ajax-type="pn_finance_manager_asset_edit" title="<?php esc_attr_e('Edit Asset', 'pn-finance-manager'); ?>">
												<i class="material-icons-outlined">edit</i> <?php esc_html_e('Edit Asset', 'pn-finance-manager'); ?>
											</a>
										</div>
									<?php if ($asset['stock_data']): ?>
										<div class="pn-finance-manager-performance-metrics pn-finance-manager-stock-detail-metrics">
											<div class="pn-finance-manager-metric-row">
												<div class="pn-finance-manager-metric-item">
													<span class="pn-finance-manager-metric-label"><?php echo esc_html($type_key === 'cryptocurrencies' ? __('Amount', 'pn-finance-manager') : __('Shares', 'pn-finance-manager')); ?></span>
													<span class="pn-finance-manager-metric-value"><?php echo esc_html(number_format($asset['shares'], 2)); ?></span>
												</div>
												<div class="pn-finance-manager-metric-item">
													<span class="pn-finance-manager-metric-label"><?php esc_html_e('Purchase Price', 'pn-finance-manager'); ?></span>
													<span class="pn-finance-manager-metric-value">
														<?php if ($asset['purchase_price'] > 0): ?>
															<?php
															$fmt_purchase = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency(PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($asset['purchase_price'], $currency), $currency);
															echo esc_html($fmt_purchase['full']);
															?>
														<?php else: ?>
															<button class="<?php echo esc_attr($type_key === 'cryptocurrencies' ? 'pn-finance-manager-fetch-crypto-purchase-price' : 'pn-finance-manager-fetch-purchase-price'); ?> pn-finance-manager-btn pn-finance-manager-btn-mini"
																data-asset-id="<?php echo esc_attr($asset['id']); ?>"
																data-symbol="<?php echo esc_attr(strtoupper($asset['symbol'])); ?>">
																<i class="material-icons-outlined">sync</i> <?php esc_html_e('Fetch price', 'pn-finance-manager'); ?>
															</button>
														<?php endif; ?>
													</span>
												</div>
											...
											<div class="pn-finance-manager-metric-row">
												<div class="pn-finance-manager-metric-item">
													<span class="pn-finance-manager-metric-label"><?php esc_html_e('Current Price', 'pn-finance-manager'); ?></span>
													<span class="pn-finance-manager-metric-value">
														<?php
														$fmt_current_price = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency(PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($asset['current_value'], $currency), $currency);
														echo esc_html($fmt_current_price['full']);
														?>
													</span>
												</div>
												<div class="pn-finance-manager-metric-item">
													<span class="pn-finance-manager-metric-label"><?php esc_html_e('Price Variation', 'pn-finance-manager'); ?></span>
													<?php
													...
													?>
													<span class="pn-finance-manager-metric-value <?php echo esc_attr($price_variation >= 0 ? 'positive' : 'negative'); ?>">
											...
											<div class="pn-finance-manager-metric-row">
												<div class="pn-finance-manager-metric-item">
													...
													<span class="pn-finance-manager-metric-value">
														<?php
														$fmt_invested = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency(PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($asset['total_invested'], $currency), $currency);
												...
												<div class="pn-finance-manager-metric-item">
													<span class="pn-finance-manager-metric-label"><?php esc_html_e('Profit/Loss', 'pn-finance-manager'); ?></span>
													<span class="pn-finance-manager-metric-value <?php echo esc_attr($asset['profit_loss'] >= 0 ? 'positive' : 'negative'); ?>">
														<?php
														$converted_profit = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency(PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($asset['profit_loss'], $currency), $currency);
											...
											<div class="pn-finance-manager-metric-row">
												<div class="pn-finance-manager-metric-item">
													<span class="pn-finance-manager-metric-label"><?php esc_html_e('Sale Date', 'pn-finance-manager'); ?></span>
													<span class="pn-finance-manager-metric-value">
														<?php echo !empty($asset['sold_date']) ? esc_html(date_i18n(get_option('date_format'), strtotime($asset['sold_date']))) : '—'; ?>
													</span>
												</div>
												<div class="pn-finance-manager-metric-item">
													<span class="pn-finance-manager-metric-label"><?php esc_html_e('Sale Price', 'pn-finance-manager'); ?></span>
													<span class="pn-finance-manager-metric-value">
														<?php
														$fmt_sold = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency(PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($asset['sold_price'], $currency), $currency);
										...
										<?php if (!empty($asset['price_history']) && empty($asset['is_sold'])): ?>
											<?php
											$stock_chart_id = 'pn-finance-manager-stock-chart-' . esc_attr($asset['id']);
											$chart_symbol = PN_FINANCE_MANAGER_Data::pn_finance_manager_get_currency_symbol($currency);
											$stock_history_data = [];
											foreach ($asset['price_history'] as $row) {
												if (!empty($row['recorded_date']) && isset($row['price'])) {
													$stock_history_data[] = [
														'date' => $row['recorded_date'],
														'price' => round(PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd(floatval($row['price']), $currency), 2)
													];
												}
											}
											if (!empty($stock_history_data)):
											?>
											<div class="pn-finance-manager-stock-evolution-chart">
												<button type="button" class="pn-finance-manager-chart-fullscreen-btn pn-finance-manager-btn pn-finance-manager-btn-mini" title="<?php esc_attr_e('Fullscreen', 'pn-finance-manager'); ?>">
													<i class="material-icons-outlined">fullscreen</i>
												</button>
												<div class="pn-finance-manager-chart-container">
										...
										<?php endif; ?>
									<?php else: ?>
										<div class="pn-finance-manager-stock-no-data">
											<?php esc_html_e('No current data available', 'pn-finance-manager'); ?>
										</div>
									<?php endif; ?>
									</div>
								</div>
						...
						<div class="pn-finance-manager-breakdown-charts-row" style="margin-top:30px;">
							<div class="pn-finance-manager-breakdown-chart-item">
								<h4><?php esc_html_e('Stocks Breakdown', 'pn-finance-manager'); ?></h4>
								<div class="pn-finance-manager-chart-container">
									<canvas id="pn-finance-manager-stocks-breakdown-doughnut" width="300" height="300"></canvas>
								</div>
							</div>
							<script>
							jQuery(document).ready(function($) {
								function drawStocksBreakdownChart() {
									...
									var ctx = document.getElementById('pn-finance-manager-stocks-breakdown-doughnut').getContext('2d');
									var colors = <?php echo json_encode($stocks_chart_colors); ?>;
									var dataValues = <?php echo json_encode($stocks_chart_values); ?>;
									var bgColors = dataValues.map(function(_, i) { return colors[i % colors.length]; });
									new Chart(ctx, {
										type: 'doughnut',
										data: {
											labels: <?php echo json_encode($stocks_chart_labels); ?>,
											datasets: [{
												data: dataValues,
												backgroundColor: bgColors,
												borderWidth: 2,
												...
												tooltip: {
													callbacks: {
														label: function(context) {
															...
															return context.label + ': <?php echo esc_js($breakdown_symbol); ?>' + context.parsed.toFixed(2) + ' (' + percentage + '%)';
														}
													}
												}
											}
							...
							<div class="pn-finance-manager-breakdown-chart-item">
								<h4><?php esc_html_e('Sector Breakdown', 'pn-finance-manager'); ?></h4>
								<div class="pn-finance-manager-chart-container">
									<canvas id="pn-finance-manager-sector-breakdown-doughnut" width="300" height="300"></canvas>
								</div>
							</div>
							<script>
							jQuery(document).ready(function($) {
								function drawSectorBreakdownChart() {
									...
									var ctx = document.getElementById('pn-finance-manager-sector-breakdown-doughnut').getContext('2d');
									var colors = <?php echo json_encode($sector_chart_colors); ?>;
									var dataValues = <?php echo json_encode($sector_chart_values); ?>;
									var bgColors = dataValues.map(function(_, i) { return colors[i % colors.length]; });
									new Chart(ctx, {
										type: 'doughnut',
										data: {
											labels: <?php echo json_encode($sector_chart_labels); ?>,
											datasets: [{
												data: dataValues,
												backgroundColor: bgColors,
												borderWidth: 2,
							...
							<div class="pn-finance-manager-breakdown-chart-item">
								<h4><?php esc_html_e('Country Diversification', 'pn-finance-manager'); ?></h4>
								<div class="pn-finance-manager-chart-container">
									<canvas id="pn-finance-manager-country-breakdown-doughnut" width="300" height="300"></canvas>
								</div>
							</div>
							<script>
							jQuery(document).ready(function($) {
								function drawCountryBreakdownChart() {
									...
									var ctx = document.getElementById('pn-finance-manager-country-breakdown-doughnut').getContext('2d');
									var colors = <?php echo json_encode($country_chart_colors); ?>;
									var dataValues = <?php echo json_encode($country_chart_values); ?>;
									var bgColors = dataValues.map(function(_, i) { return colors[i % colors.length]; });
									new Chart(ctx, {
										type: 'doughnut',
										data: {
											labels: <?php echo json_encode($country_chart_labels); ?>,
											datasets: [{
												data: dataValues,
												backgroundColor: bgColors,
												borderWidth: 2,
						...
						<div class="pn-finance-manager-portfolio-performance-chart" style="margin-top: 30px;">
							<h4><?php esc_html_e('Stock Portfolio Performance', 'pn-finance-manager'); ?></h4>
							<div class="pn-finance-manager-performance-metrics" style="margin-bottom:10px;">
								<div class="pn-finance-manager-metric-row">
									<div class="pn-finance-manager-metric-item">
										<span class="pn-finance-manager-metric-label"><?php esc_html_e('Total Invested', 'pn-finance-manager'); ?></span>
										<span class="pn-finance-manager-metric-value"><?php echo esc_html($fmt_sp_inv['full']); ?></span>
									</div>
									<div class="pn-finance-manager-metric-item">
										<span class="pn-finance-manager-metric-label"><?php esc_html_e('Current Value', 'pn-finance-manager'); ?></span>
										<span class="pn-finance-manager-metric-value"><?php echo esc_html($fmt_sp_cur['full']); ?></span>
									</div>
									<div class="pn-finance-manager-metric-item">
										<span class="pn-finance-manager-metric-label"><?php esc_html_e('Total Return', 'pn-finance-manager'); ?></span>
										<span class="pn-finance-manager-metric-value <?php echo esc_attr($stocks_perf_return >= 0 ? 'positive' : 'negative'); ?>">
							...
							<div class="pn-finance-manager-chart-container">
								...
								<canvas id="pn-finance-manager-stocks-perf-chart"
									data-chart-data="<?php echo esc_attr(json_encode($stocks_perf_chart_data)); ?>"
									data-chart-symbol="<?php echo esc_attr(PN_FINANCE_MANAGER_Data::pn_finance_manager_get_currency_symbol($currency)); ?>"
									data-chart-label="<?php esc_attr_e('Stock Portfolio', 'pn-finance-manager'); ?>"
						...
						jQuery(document).ready(function($) {
							function drawStocksPerfChart() {
								...
								var canvas = document.getElementById('pn-finance-manager-stocks-perf-chart');
										...
										datasets: [
											{
												...
												backgroundColor: '<?php echo esc_js($primary_color_rgba); ?>',
												fill: true,
												tension: 0.3,
												pointRadius: 2
											},
											{
												label: '<?php echo esc_js(__('Invested', 'pn-finance-manager')); ?>',
						...
						<div class="pn-finance-manager-portfolio-performance-chart" style="margin-top: 30px;">
							<h4><?php esc_html_e('Crypto Portfolio Performance', 'pn-finance-manager'); ?></h4>
							<div class="pn-finance-manager-performance-metrics" style="margin-bottom:10px;">
								<div class="pn-finance-manager-metric-row">
									<div class="pn-finance-manager-metric-item">
										<span class="pn-finance-manager-metric-label"><?php esc_html_e('Total Invested', 'pn-finance-manager'); ?></span>
										<span class="pn-finance-manager-metric-value"><?php echo esc_html($fmt_cp_inv['full']); ?></span>
									</div>
									<div class="pn-finance-manager-metric-item">
										<span class="pn-finance-manager-metric-label"><?php esc_html_e('Current Value', 'pn-finance-manager'); ?></span>
										<span class="pn-finance-manager-metric-value"><?php echo esc_html($fmt_cp_cur['full']); ?></span>
									</div>
									<div class="pn-finance-manager-metric-item">
										<span class="pn-finance-manager-metric-label"><?php esc_html_e('Total Return', 'pn-finance-manager'); ?></span>
										<span class="pn-finance-manager-metric-value <?php echo esc_attr($crypto_perf_return >= 0 ? 'positive' : 'negative'); ?>">
							...
							<div class="pn-finance-manager-chart-container">
								...
								<canvas id="pn-finance-manager-crypto-perf-chart"
									data-chart-data="<?php echo esc_attr(json_encode($crypto_perf_chart_data)); ?>"
									data-chart-symbol="<?php echo esc_attr(PN_FINANCE_MANAGER_Data::pn_finance_manager_get_currency_symbol($currency)); ?>"
									data-chart-label="<?php esc_attr_e('Crypto Portfolio', 'pn-finance-manager'); ?>"
						...
						jQuery(document).ready(function($) {
							function drawCryptoPerfChart() {
								...
								var canvas = document.getElementById('pn-finance-manager-crypto-perf-chart');
								var ctx = canvas.getContext('2d');
								var chartData = JSON.parse(canvas.getAttribute('data-chart-data'));
								var symbol = canvas.getAttribute('data-chart-symbol');
								var invested = parseFloat(canvas.getAttribute('data-chart-invested'));
						...
						<div class="pn-finance-manager-breakdown-charts-row" style="margin-top:30px;">
							<div class="pn-finance-manager-breakdown-chart-item">
								<h4><?php esc_html_e('Crypto Breakdown', 'pn-finance-manager'); ?></h4>
								<div class="pn-finance-manager-chart-container">
									<canvas id="pn-finance-manager-crypto-breakdown-doughnut" width="300" height="300"></canvas>
								</div>
							</div>
							<script>
							jQuery(document).ready(function($) {
								function drawCryptoBreakdownChart() {
									...
									var ctx = document.getElementById('pn-finance-manager-crypto-breakdown-doughnut').getContext('2d');
									var colors = <?php echo json_encode($crypto_chart_colors); ?>;
									var dataValues = <?php echo json_encode($crypto_chart_values); ?>;
									var bgColors = dataValues.map(function(_, i) { return colors[i % colors.length]; });
									new Chart(ctx, {
										type: 'doughnut',
										data: {
											labels: <?php echo json_encode($crypto_chart_labels); ?>,
											datasets: [{
												data: dataValues,
												backgroundColor: bgColors,
												borderWidth: 2,
					...
					<?php elseif ($type_key === 'real_estate'): ?>
								...
								<div class="pn-finance-manager-stock-card<?php echo esc_attr(!empty($asset['is_sold']) ? ' pn-finance-manager-stock-card-sold' : ''); ?> pn-finance-manager-asset" data-pn_finance_manager_asset-id="<?php echo esc_attr($asset['id']); ?>" data-sort-name="<?php echo esc_attr($asset['title']); ?>" data-sort-value="<?php echo esc_attr($asset['current_total_value']); ?>" data-sort-pl="<?php echo esc_attr($asset['profit_loss']); ?>" data-sort-date="<?php echo esc_attr($asset['purchase_date']); ?>" data-sort-sold="<?php echo esc_attr(!empty($asset['is_sold']) ? '1' : '0'); ?>">
									<div class="pn-finance-manager-stock-header pn-finance-manager-stock-toggle">
										...
										<div>
											<?php if (isset($asset['ownership_percent']) && $asset['ownership_percent'] < 100): ?>
												<span class="pn-finance-manager-stock-symbol"><?php echo esc_html(number_format($asset['ownership_percent'], 1)); ?>%</span>
											<?php endif; ?>
											<i class="material-icons-outlined pn-finance-manager-stock-toggle-icon">expand_more</i>
										</div>
									</div>
									...
									<div class="pn-finance-manager-stock-card-summary pn-finance-manager-mb-10">
										...
										<?php elseif ($asset['profit_loss'] != 0): ?>
											<div class="pn-finance-manager-stock-change <?php echo esc_attr($asset['profit_loss'] >= 0 ? 'positive' : 'negative'); ?>">
										...
										<div class="pn-finance-manager-performance-metrics pn-finance-manager-stock-detail-metrics">
											<div class="pn-finance-manager-metric-row">
												<div class="pn-finance-manager-metric-item">
													...
													<span class="pn-finance-manager-metric-value">
														<?php
														$fmt_current = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency(PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($asset['current_value'], $currency), $currency);
												...
												<div class="pn-finance-manager-metric-item">
													...
													<span class="pn-finance-manager-metric-value">
														<?php
														$fmt_purchase = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency(PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($asset['purchase_value'], $currency), $currency);
														echo esc_html($fmt_purchase['full']);
														?>
													</span>
												</div>
											...
											<div class="pn-finance-manager-metric-row">
												<div class="pn-finance-manager-metric-item">
													<span class="pn-finance-manager-metric-label"><?php esc_html_e('Ownership', 'pn-finance-manager'); ?></span>
													<span class="pn-finance-manager-metric-value"><?php echo esc_html(number_format($asset['ownership_percent'], 2)); ?>%</span>
												</div>
												<div class="pn-finance-manager-metric-item">
													<span class="pn-finance-manager-metric-label"><?php esc_html_e('Your Share', 'pn-finance-manager'); ?></span>
													<span class="pn-finance-manager-metric-value">
														<?php echo esc_html($converted['full']); ?>
													</span>
												</div>
											...
											<div class="pn-finance-manager-metric-row">
												...
												<div class="pn-finance-manager-metric-item">
													...
													<span class="pn-finance-manager-metric-value">
														<?php
														$fmt_sold_re = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency(PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($asset['current_value'], $currency), $currency);
						...
						<div class="pn-finance-manager-breakdown-charts-row" style="margin-top:30px;">
							<div class="pn-finance-manager-breakdown-chart-item">
								<h4><?php esc_html_e('Real Estate Breakdown', 'pn-finance-manager'); ?></h4>
								<div class="pn-finance-manager-chart-container">
									<canvas id="pn-finance-manager-realestate-breakdown-doughnut" width="300" height="300"></canvas>
								</div>
							</div>
							<script>
							jQuery(document).ready(function($) {
								function drawRealEstateBreakdownChart() {
									...
									var ctx = document.getElementById('pn-finance-manager-realestate-breakdown-doughnut').getContext('2d');
									var colors = <?php echo json_encode($re_chart_colors); ?>;
									var dataValues = <?php echo json_encode($re_chart_values); ?>;
									var bgColors = dataValues.map(function(_, i) { return colors[i % colors.length]; });
									new Chart(ctx, {
										type: 'doughnut',
										data: {
											labels: <?php echo json_encode($re_chart_labels); ?>,
											datasets: [{
												data: dataValues,
												backgroundColor: bgColors,
												borderWidth: 2,
						...
						<?php endif; ?>
					<?php else: ?>
						<div class="pn-finance-manager-other-assets">
							<?php foreach ($type_data['assets'] as $asset): ?>
								<div class="pn-finance-manager-other-asset-card pn-finance-manager-asset" data-pn_finance_manager_asset-id="<?php echo esc_attr($asset['id']); ?>" data-sort-name="<?php echo esc_attr($asset['title']); ?>" data-sort-value="<?php echo esc_attr($asset['current_value'] ?? 0); ?>" data-sort-date="<?php echo esc_attr($asset['purchase_date']); ?>">
									<div class="pn-finance-manager-other-asset-header">
										<h4><?php echo esc_html($asset['title']); ?></h4>
										<a href="#" class="pn-finance-manager-popup-open-ajax pn-finance-manager-text-decoration-none pn-finance-manager-edit-asset-link" data-pn-finance-manager-popup-id="pn-finance-manager-popup-pn_finance_manager_asset-edit" data-pn-finance-manager-ajax-type="pn_finance_manager_asset_edit" title="<?php esc_attr_e('Edit Asset', 'pn-finance-manager'); ?>">
									...
									<div class="pn-finance-manager-asset-meta">
										<span class="pn-finance-manager-purchase-date">
											<?php esc_html_e('Purchased:', 'pn-finance-manager'); ?>
											<?php echo esc_html($asset['purchase_date'] ? date_i18n(get_option('date_format'), strtotime($asset['purchase_date'])) : __('Unknown', 'pn-finance-manager')); ?>
										</span>
									</div>
								</div>
							<?php endforeach; ?>
					...
					<?php endif; ?>
					</div><!-- .pn-finance-manager-section-body (category) -->
				</div>
			<?php endforeach; ?>
			</div><!-- .pn-finance-manager-section-body (Assets) -->

			<div class="pn-finance-manager-section-header">
				<h2 class="pn-finance-manager-section-toggle collapsed"><?php esc_html_e('Liabilities', 'pn-finance-manager'); ?> <i class="material-icons-outlined pn-finance-manager-section-toggle-icon">expand_less</i></h2>
				<?php if (is_user_logged_in()) : ?>
				<a href="#" class="pn-finance-manager-popup-open-ajax pn-finance-manager-text-decoration-none" data-pn-finance-manager-popup-id="pn-finance-manager-popup-pn_finance_manager_liability-add" data-pn-finance-manager-ajax-type="pn_finance_manager_liability_new">
					<i class="material-icons-outlined pn-finance-manager-cursor-pointer pn-finance-manager-vertical-align-middle pn-finance-manager-tooltip pn-finance-manager-color-main-0" title="<?php esc_attr_e('Add new Liability', 'pn-finance-manager'); ?>">add_circle</i>
				</a>
				<?php endif; ?>
			</div>
			<div class="pn-finance-manager-section-body">
			...
			<?php if (!empty($user_liabilities)): ?>
				<?php foreach ($user_liabilities as $type_key => $type_data): ?>
					<div class="pn-finance-manager-asset-category">
						<h3 class="pn-finance-manager-section-toggle collapsed"><?php echo esc_html($type_data['label']); ?>
							<span class="pn-finance-manager-asset-count">(<?php echo esc_html($type_data['count']); ?>)</span>
							<i class="material-icons-outlined pn-finance-manager-section-toggle-icon">expand_less</i>
						</h3>
						<div class="pn-finance-manager-section-body">
						<div class="pn-finance-manager-sort-bar">
							<select class="pn-finance-manager-sort-select">
								<option value="name"><?php esc_html_e('Name', 'pn-finance-manager'); ?></option>
								<option value="balance"><?php esc_html_e('Balance', 'pn-finance-manager'); ?></option>
								<option value="date"><?php esc_html_e('Date', 'pn-finance-manager'); ?></option>
							</select>
							<button type="button" class="pn-finance-manager-sort-dir-btn" data-dir="asc" title="<?php esc_attr_e('Sort ascending', 'pn-finance-manager'); ?>">
								<i class="material-icons-outlined">arrow_upward</i>
							</button>
						...
						<div class="pn-finance-manager-stocks-grid">
							<?php foreach ($type_data['liabilities'] as $liability): ?>
								<?php
								$converted_balance = PN_FINANCE_MANAGER_Data::pn_finance_manager_format_currency(
									PN_FINANCE_MANAGER_Data::pn_finance_manager_convert_from_usd($liability['balance'], $currency),
									$currency
								);
								?>
								<div class="pn-finance-manager-stock-card" data-pn_finance_manager_asset-id="<?php echo esc_attr($liability['id']); ?>" data-sort-name="<?php echo esc_attr($liability['title']); ?>" data-sort-balance="<?php echo esc_attr($liability['balance']); ?>" data-sort-date="<?php echo esc_attr($liability['date']); ?>">
									<div class="pn-finance-manager-stock-header pn-finance-manager-stock-toggle">
									...
									<div class="pn-finance-manager-stock-card-summary pn-finance-manager-mb-10">
										<div class="pn-finance-manager-stock-card-summary-value negative">
											<?php echo esc_html($converted_balance['full']); ?>
										</div>
									</div>
									<div class="pn-finance-manager-stock-body" style="display:none;">
										<div class="pn-finance-manager-stock-body-actions">
											<a href="#" class="pn-finance-manager-popup-open-ajax pn-finance-manager-text-decoration-none pn-finance-manager-edit-asset-link" data-pn-finance-manager-popup-id="pn-finance-manager-popup-pn_finance_manager_liability-edit" data-pn-finance-manager-ajax-type="pn_finance_manager_liability_edit" title="<?php esc_attr_e('Edit Liability', 'pn-finance-manager'); ?>">
												<i class="material-icons-outlined">edit</i> <?php esc_html_e('Edit Liability', 'pn-finance-manager'); ?>
											</a>
										</div>
										<div class="pn-finance-manager-performance-metrics pn-finance-manager-stock-detail-metrics">
											<div class="pn-finance-manager-metric-row">
												<div class="pn-finance-manager-metric-item">
													<span class="pn-finance-manager-metric-label"><?php esc_html_e('Balance', 'pn-finance-manager'); ?></span>
													<span class="pn-finance-manager-metric-value negative">
														<?php echo esc_html($converted_balance['full']); ?>
													</span>
												</div>
												...
												<div class="pn-finance-manager-metric-item">
													<span class="pn-finance-manager-metric-label"><?php esc_html_e('Date', 'pn-finance-manager'); ?></span>
													<span class="pn-finance-manager-metric-value">
						...
						</div><!-- .pn-finance-manager-section-body (liability category) -->
					</div>
				<?php endforeach; ?>
			<?php endif; ?>
			</div><!-- .pn-finance-manager-section-body (Liabilities) -->
		</div>
		<?php
	 ...
	 */
	public function pn_finance_manager_clean_old_stock_price_data($days_to_keep = 365) {
		global $wpdb;
		...
		$options_with_history = $wpdb->get_results(
			...
			FROM {$wpdb->options} 
			WHERE option_name LIKE 'pn_finance_manager_stock_price_history_%'"
		);
	 ...
	 */
	public function pn_finance_manager_get_all_stock_symbols() {
		global $wpdb;
		...
		foreach ($options as $option) {
			$symbol = str_replace('pn_finance_manager_stock_price_history_', '', $option->option_name);
			$symbols[] = $symbol;
		}
	 ...
	 * @return   array                 Array of stock symbols.
	 */
	public function pn_finance_manager_get_user_stock_symbols($user_id) {
	 ...
	 */
	public function pn_finance_manager_get_cache_size() {
		global $wpdb;
	 ...
	 */
	public function pn_finance_manager_get_cache_stats() {
		global $wpdb;
		
		$symbols = $this->pn_finance_manager_get_all_stock_symbols();
		$cache_size = $this->pn_finance_manager_get_cache_size();
	 ...
	 */
	public function pn_finance_manager_get_stock_symbols_cache_stats() {
		$cached_symbols = get_option('pn_finance_manager_stock_symbols_cache', []);
		$last_update = get_option('pn_finance_manager_stock_symbols_last_update', 0);
		$cache_expiry = get_option('pn_finance_manager_stock_symbols_cache_expiry', 24 * HOUR_IN_SECONDS);
		...
		return [
			...
			'last_update_formatted' => $last_update ? date_i18n(get_option('date_format') . ' @ ' . get_option('time_format'), $last_update) : __('Never', 'pn-finance-manager'),
			'cache_expiry' => $cache_expiry,
			'is_valid' => $is_valid,
			'next_expiry' => $last_update + $cache_expiry,
			'next_expiry_formatted' => $last_update ? date_i18n(get_option('date_format') . ' @ ' . get_option('time_format'), $last_update + $cache_expiry) : __('N/A', 'pn-finance-manager'),
	 ...
	 * @return   bool                  Success status.
	 */
	public function pn_finance_manager_clear_stock_symbols_cache() {
		$result1 = delete_option('pn_finance_manager_stock_symbols_cache');
		$result2 = delete_option('pn_finance_manager_stock_symbols_last_update');
		$result3 = delete_option('pn_finance_manager_stock_symbols_cache_expiry');
		
		error_log('PnFinanceManager: Stock symbols cache cleared.');
	 ...
	 */
	public function pn_finance_manager_force_update_stock_symbols_cache() {
		// Clear existing cache first
		$this->pn_finance_manager_clear_stock_symbols_cache();
		
		// Force update
		$result = $this->pn_finance_manager_update_stock_symbols_cache();
		
		if ($result && is_array($result)) {
			error_log('PnFinanceManager: Stock symbols cache force updated successfully. Total symbols: ' . count($result));
		} else {
			error_log('PnFinanceManager: Failed to force update stock symbols cache.');
		}
	 ...
	 */
	public function pn_finance_manager_get_stock_symbols_cache_info() {
		$stats = $this->pn_finance_manager_get_stock_symbols_cache_stats();

		ob_start();
		?>
		<div class="pn-finance-manager-cache-info">
			<h3><?php esc_html_e('Stock Symbols Cache Information', 'pn-finance-manager'); ?></h3>
			<table class="widefat">
				<tbody>
					<tr>
						<td><strong><?php esc_html_e('Symbols in Cache:', 'pn-finance-manager'); ?></strong></td>
					...
					<tr>
						...
						<td>
							<span class="<?php echo esc_attr($stats['is_valid'] ? 'pn-finance-manager-status-valid' : 'pn-finance-manager-status-expired'); ?>">
								<?php echo esc_html($stats['is_valid'] ? __('Valid', 'pn-finance-manager') : __('Expired', 'pn-finance-manager')); ?>
							</span>
						</td>
					</tr>
					<tr>
						<td><strong><?php esc_html_e('Next Expiry:', 'pn-finance-manager'); ?></strong></td>
	 ...
	 */
	public function pn_finance_manager_get_crypto_symbols_for_form() {
		$cached_symbols = get_option('pn_finance_manager_crypto_symbols_cache', []);
		$last_update = get_option('pn_finance_manager_crypto_symbols_last_update', 0);
		...
		$update_result = $this->pn_finance_manager_update_crypto_symbols_cache();
		...
		return $this->pn_finance_manager_get_popular_crypto_symbols();
	}
	 ...
	 */
	public function pn_finance_manager_update_crypto_symbols_cache() {
		$symbols = $this->pn_finance_manager_get_coingecko_coins_list();

		if (!empty($symbols) && is_array($symbols)) {
			update_option('pn_finance_manager_crypto_symbols_cache', $symbols, false);
			update_option('pn_finance_manager_crypto_symbols_last_update', current_time('timestamp'), false);
			return $symbols;
		}
	 ...
	 */
	private function pn_finance_manager_get_coingecko_coins_list() {
		...
		if (is_wp_error($response)) {
			error_log('PnFinanceManager: CoinGecko coins/markets API error: ' . $response->get_error_message());
			return $this->pn_finance_manager_get_popular_crypto_symbols();
		}

		$http_code = wp_remote_retrieve_response_code($response);
		if ($http_code !== 200) {
			error_log('PnFinanceManager: CoinGecko API returned HTTP ' . $http_code);
			return $this->pn_finance_manager_get_popular_crypto_symbols();
		}
		...
		if (empty($data) || !is_array($data)) {
			error_log('PnFinanceManager: CoinGecko API returned empty or invalid data.');
			return $this->pn_finance_manager_get_popular_crypto_symbols();
		}
		...
		update_option('pn_finance_manager_crypto_id_to_ticker', $id_to_ticker, false);
	 ...
	 */
	private function pn_finance_manager_get_popular_crypto_symbols() {
		$symbols = [
			'bitcoin' => 'BTC - Bitcoin',
	 ...
	 */
	public function pn_finance_manager_get_crypto_data($coin_id) {
		if (empty($coin_id)) {
			return false;
		}

		$cache_key = 'pn_finance_manager_crypto_data_' . $coin_id;
		$cached_data = get_transient($cache_key);
		if ($cached_data !== false) {
			return $cached_data;
		}

		$data = $this->pn_finance_manager_get_coingecko_price($coin_id);
	 ...
	 * @return   array|false           Crypto data or false on failure.
	 */
	private function pn_finance_manager_get_coingecko_price($coin_id) {
		...
		if (is_wp_error($response)) {
			error_log('PnFinanceManager: CoinGecko simple/price error: ' . $response->get_error_message());
			return false;
		}
		...
		if (empty($data) || !isset($data[$coin_id])) {
			error_log('PnFinanceManager: CoinGecko simple/price returned no data for ' . $coin_id);
			return false;
		}
	 ...
	 * @return   array|false           Historical data or false on failure.
	 */
	public function pn_finance_manager_get_historical_crypto_data($coin_id, $days = 30) {
		if (empty($coin_id)) {
			return false;
		}

		$existing_data = $this->pn_finance_manager_get_crypto_price_history($coin_id, $days);
		$last_update = get_option('pn_finance_manager_historical_crypto_data_last_update_' . $coin_id, '');
		...
		$historical_data = $this->pn_finance_manager_get_coingecko_historical($coin_id, $days);

		if ($historical_data) {
			$this->pn_finance_manager_store_crypto_historical_data($coin_id, $historical_data);
			update_option('pn_finance_manager_historical_crypto_data_last_update_' . $coin_id, current_time('mysql'), false);
			return $historical_data;
		}
	 ...
	 * @return   array|false           Historical data or false on failure.
	 */
	private function pn_finance_manager_get_coingecko_historical($coin_id, $days) {
		...
		if (is_wp_error($response)) {
			error_log('PnFinanceManager: CoinGecko market_chart error: ' . $response->get_error_message());
			return false;
		}
		...
		if (empty($data['prices']) || !is_array($data['prices'])) {
			error_log('PnFinanceManager: CoinGecko market_chart returned empty data for ' . $coin_id);
			return false;
		}
	 ...
	 * @return   bool                        Success status.
	 */
	private function pn_finance_manager_store_crypto_historical_data($coin_id, $historical_data) {
		if (empty($historical_data)) {
			return false;
		}

		$option_key = 'pn_finance_manager_crypto_price_history_' . $coin_id;
	 ...
	 * @return   bool                  Success status.
	 */
	public function pn_finance_manager_record_crypto_price($coin_id, $data) {
		$option_key = 'pn_finance_manager_crypto_price_history_' . $coin_id;
		$today = current_time('Y-m-d');
	 ...
	 * @return   array                 Historical price data.
	 */
	public function pn_finance_manager_get_crypto_price_history($coin_id, $days = 30) {
		$option_key = 'pn_finance_manager_crypto_price_history_' . $coin_id;
		$price_history = get_option($option_key, []);
	 ...
	 * @return   array|false           Price data or false if not found.
	 */
	public function pn_finance_manager_get_crypto_price_for_date($coin_id, $date) {
		$option_key = 'pn_finance_manager_crypto_price_history_' . $coin_id;
		$price_history = get_option($option_key, []);
	 ...
	 * @since    1.1.0
	 */
	public function pn_finance_manager_update_crypto_symbols_from_api_cron() {
		$result = $this->pn_finance_manager_update_crypto_symbols_cache();
		if ($result && is_array($result)) {
			error_log('PnFinanceManager: Crypto symbols updated via cron. Total: ' . count($result));
		} else {
			error_log('PnFinanceManager: Failed to update crypto symbols via cron.');
		}
	}
	 ...
	 * @since    1.1.0
	 */
	public function pn_finance_manager_daily_crypto_price_recording() {
		...
		foreach ($unique_coins as $coin_id) {
			...
			if ($crypto_data) {
				$result = $this->pn_finance_manager_record_crypto_price($coin_id, $crypto_data);
				if ($result) {
					$recorded_count++;
				}
			}
		...
		error_log('PnFinanceManager: Daily crypto price recording completed. Recorded: ' . $recorded_count . '/' . count($unique_coins));
	}
}