...
if (class_exists('Polylang')) {
	$liabilities = get_posts(['numberposts' => -1, 'fields' => 'ids', 'post_type' => 'pnfm_liability', 'lang' => pll_current_language(), 'post_status' => ['publish'], 'order' => 'DESC', ]);
} else {
	$liabilities = get_posts(['numberposts' => -1, 'fields' => 'ids', 'post_type' => 'pnfm_liability', 'post_status' => ['publish'], 'order' => 'DESC', ]);
}
...
?>
<div class="pn-finance-manager-liability-archive">
  <h1><?php esc_html_e('Liabilities', 'pn-finance-manager'); ?></h1>
  <div class="pn-finance-manager-liability-list">
    <?php if (!empty($liabilities)) : ?>
      <?php foreach ($liabilities as $liability_id) : ?>
        <div class="pn-finance-manager-liability-archive-item">
      ...
      <?php endforeach; ?>
    <?php else : ?>
      <p><?php esc_html_e('No liabilities found.', 'pn-finance-manager'); ?></p>
    <?php endif; ?>
  </div>
</div>
<?php
